...
### 2026-07-20 (egg speed-up badge orientation and SpecialShop category redesign)
- Fixed the maturing-egg rewarded badge at its transform source: dynamically created ad artwork now lives under the same `buyInfo` container as the readable action text and inherits its 180-degree world-UI correction. The field prompt no longer supplies the mirrored root as an explicit parent, and its top-right offset changed from `+10` to `-8` so the 42x42 icon and `-30` label remain inside the action plate instead of overlapping the `E` key.
...
### 2026-07-21 (Tutorial world arrow paths and UI hand reliability)
- Replaced Tutorial V2's screen-edge world arrow with two runtime `ArrowLine` guides. World targets now receive a moving yellow/cyan arrow texture path from the player, projected slightly above the ground; screen-space UI targets use only `UIHandPointer`.
- Repaired the dormant `AutoArrowLine`: it resolves `G.Player` when the player appears, supports target replacement, explicit endpoints and clean stop/disable behavior. `ArrowLine` now refreshes grounded endpoints at a bounded rate, uses unscaled animation time and destroys its private runtime material.
...
### 2026-07-21 (Tutorial distant path and conveyor-step targeting)
- Capped Tutorial V2 world guides to a `14 m` directional segment near the player instead of stretching the arrow texture across the full distance. Grounded paths now contain sampled points, while the projection mask is limited to Default/Ground so players, creatures and interaction colliders cannot deform the ribbon. Removed the second distance multiplier from `LineTextureMode.Tile`: baked UV inspection confirmed that Unity already emits a world-distance UV span (`10 m -> U 0..10`), so material scale is now the constant inverse arrow width.
- Split step 10 guidance by economy state. While coins are below the live `NextUpgradePriceCoins` value, both world target and highlight are null; the task only reports savings progress. Once affordable, the world target is `Conveyor.UpgradeInteractionTarget`.
- Exposed the actual conveyor interaction transform through `UpgradeInteractionTarget`, which resolves to `LevelsArea` (the collider/listener invoking `_OnPlayerEnter`) rather than the conveyor model root. The UI-open state still points at the coin purchase button.
- Unity recompiled without errors and the 10-step/66-key Tutorial validator passed. Live Bridge returned `price=15000|lowTarget=null|readyTarget=LevelsArea`; a synthetic 60 m target produced a 10-point path with a 12.90 m visible flat span and material scale `0.870`, exactly `1 / 1.15` arrow width. Play Mode was stopped after verification.

### 2026-07-22 (Tutorial step 7 quiet saving phase and masked completion animation)
- Removed the income-collector world target/highlight from `ResolveBigPetPurchaseContext` while the player has fewer coins than the live `BigPetPoint.UnlockPrice`. The task continues to display dynamic savings progress, then restores its existing return-home, BigPet-area and buy-button guidance once affordable.
- Replaced the completion animation that scaled the entire 610x290 `TutorialPanel` to 110% inside an equally sized `RectMask2D`. The new 0.48-second cubic pop-in scales only `RewardBadge` from 90% to its authored size, so no animated geometry can leave the mask.
- Added a centralized stop/reset path so interrupted, disabled or destroyed views always restore the reward scale. Live Bridge at current `UnlockPrice=10000` returned all three saving-phase targets null; iterator sampling measured reward scale `0.910..1.000` and panel scale `1.000..1.000`. Play Mode was stopped after verification.

### 2026-07-27 (Sonniss GDC 2026 game-audio integration)
- Audited the project actions and the local `Sonniss.com-GDC2026-GameAudioBundle`. Added a reproducible non-destructive builder that extracts, trims, normalizes, fades, resamples and encodes selected source material without modifying the original bundle.
- Generated 84 Unity-ready Ogg/Vorbis clips at 48 kHz (2.22 MiB total). All 84 catalog clip names resolve on disk; Unity generated matching metadata. Seventy-eight short cues use Compressed In Memory at quality `0.60`, while six long ambience clips use background Streaming at quality `0.45`.
- Extended `SoundManager` with a Resources-backed event catalog, randomized pools, layered cues, per-event cooldowns, UI/SFX/environment mixer routing, 2D/3D one-shots, managed loops, global button/toggle/slider feedback and zoo ambience. Existing serialized music and fallback clips remain intact.
- Connected semantic events across movement/water/teleport, interaction holds, inventory and currency, eggs and rare/element reveals, animal and BigPet progression, claim-all, territory hammer sequence, conveyor, album, food/special shops, roulette, Tutorial V2, playtime rewards, friends/gifts/likes, gift chest and network lost/restored states.
- Added `Docs/AUDIO_GDC2026_INTEGRATION.md` with source/license provenance, rebuild instructions, runtime architecture and a manual acceptance route.
- Unity 6000.3.9f1 rebuilt `Assembly-CSharp.dll` successfully. Final Play Mode smoke in `Evgesha` returned `LOADED=84`, `MAPPED=97`, `MISSING=0`, and successful 2D UI, 3D world and streaming loop playback. No SoundManager/audio exceptions were logged; Play Mode was stopped afterward.
- Remaining acceptance is the user's listening pass for loudness, repetition and aesthetic fit, followed by the same critical flow in WebGL where browser audio activation/compression can differ.

### 2026-07-27 (first audio listening-pass revisions)
- Removed the ordinary jump cue. Water jumps retain only a natural splash layer, without the former wind/jump layer.
- Replaced timer-driven footsteps with two `Footstep` Animation Events on `Run`, `SlowRun` and `RunWithEgg`. A relay on the animated model forwards them to `TPPlayerController`; duplicate events from BlendTree mixing are limited by a short per-event cooldown. Walk animation remains at `2x`, while Shift sprint uses `3x`. Step pools now randomize pitch across `0.88..1.12`.
- Added a one-second sequential coin-collection combo in `CurrencyManager`: each collection raises pitch by `0.055` up to eight steps, then resets after a longer gap. Coin gain is slightly louder. `Claim All x2` now uses only the coin clip, and legacy fallback audio no longer fires when the central audio service merely rate-limits an event.
- Replaced the common hatch/reveal cue with the short `animal_bird` cue and animal placement with the softer `feed_offer` pop to remove the reload-like sound.
- Rebuilt water movement as a four-clip randomized pool using natural Tennessee lake and Norwegian glacial-lake recordings. Replaced the designed splash with a natural wave fragment and reduced the continuous water ambience level.
- The generated collection is now 87 Ogg/Vorbis clips at 2.26 MiB. Unity imported matching metadata, compiled successfully, and runtime verification returned `CLIPS=87`, three animated clips with two events each, a valid event relay, four water variants, `HATCH_REVEAL=animal_bird`, `CLAIM_ALL_X2=coin_gain/LAYERED=false`, and coin combo pitch `1.000 -> 1.055`.

### 2026-07-27 (offline showcase farms, BigPet elemental progression and editor flight)
- Reworked deterministic offline fallback bases so two remote neighbours remain showcase farms regardless of the local slot assignment. They open 88–100% of their land, populate 15–22 cells from the high-income end of the pet list, favor strong elemental/weight rolls, use the final conveyor levels and own a late-level elemental BigPet.
- Refit the BigPet selector collider from the visible standing pad at runtime and reduced the close hysteresis to a small buffer. The menu now closes from collider distance instead of retaining the former oversized interaction radius.
- Fixed Tutorial step 10 repeatedly forcing `ConveyorUI` back to the next unpurchased level. It performs that initial focus once per window opening and preserves subsequent manual level selection.
- Added `EditorFlightCheat`, compiled only under `UNITY_EDITOR`: double-tap Space toggles flight, WASD moves relative to the camera, Space/Ctrl changes altitude and Shift accelerates. The normal player controller is restored exactly when flight ends.
...
### 2026-08-21 (Яндекс Игры: разбор повторной модерации)
- Скачаны и покадрово просмотрены все пять вложений модерации. Замечания разложены на четыре независимых P0: адаптивная вёрстка SpecialShop, modal-арбитраж Tutorial/магазина, устранение системного медиаплеера на desktop/mobile и обязательное отображение портальной валюты с фактической SDK-ценой на IAP-карточках.
- Зафиксировано, что аудио-замечание повторилось после прежней правки. Следующая реализация должна проверить не только короткие SFX, но весь WebGL audio path: музыку/ambience, importer overrides, WebGL template и SDK-обвязку, затем подтвердить отсутствие media controls в опубликованном draft на Windows, iOS и Android.
- По магазину на видео видны пересечения/обрезка длинных текстов и промо-элементов, слишком плотная трёхколоночная mobile-сетка и completion-popup обучения поверх открытого магазина. Для IAP карточки показывают голые числа до checkout; требуется использовать фактические `PurchaseData` currency/price и исключить несовпадение карточки с платёжным окном.
- Полная постановка, ссылки на исходные вложения, матрица viewport/device и критерии приёмки добавлены в `Docs/NEXT_TASKS_HANDOFF.md` (`P0.0`, `YA-P0-01..04`), а краткие блокеры — в `Assets/_Scripts/TODO_List.md`.
- Реализация не начиналась; следующий шаг — закрывать задачи в порядке `YA-P0-03` (повторный rejection), `YA-P0-04`, `YA-P0-01`, `YA-P0-02`, после чего записать сравнительные desktop/iOS/Android видео для повторной отправки.

### 2026-08-21 (Яндекс Игры: реализация правок повторной модерации)
- Устранён WebGL media-element риск: семь ранее streaming music/ambience импортов переведены в non-streaming режим; полный аудит показал `92` AudioClip и `0` Streaming. Добавлен `WebGLAudioModerationValidator`, который блокирует WebGL build при регрессии.
- IAP-карточки теперь показывают одновременно фактические `PriceFloat` и `Currency` из Mirra product catalog. До загрузки данных отображается disabled `…`; уже открытый магазин обновляет цены раз в секунду без пересоздания UI и без локальной fallback-цены.
- SpecialShop ограничен двумя карточками в ряду, получил более крупное mobile-landscape окно и разведённые зоны title/badge/content/old price/action. TutorialView целиком скрывается, пока магазин открыт, и возвращается после закрытия с сохранённым completion/claim состоянием.
- Unity успешно импортировал и скомпилировал изменения; Play Mode открыл магазин с 26 карточками без Console errors. Контрольная development WebGL-сборка завершилась `Build Finished, Result: Success`; pre-build аудио-аудит прошёл.
- Для полного закрытия `YA-P0-01..04` остаётся загрузить новый draft и повторить moderation smoke на desktop/iOS/Android, включая RU/US/DE IAP checkout и системные media controls.

### 2026-08-21 (WebGL smoke: Mirra leaderboard 404)
- Черновик приложения `492629`, версия `0.0.0.2`, стабильно воспроизвёл системный JavaScript alert. Browser console подтвердил первопричину: пять `achievements.getScore` и пять `achievements.getLeaderboard` возвращают `404`, потому что таблицы с новыми техническими ID ещё не опубликованы на стороне Mirra/Yandex.
- Сетевой слой лидербордов временно выключен отдельным `LeaderboardService.RemoteRequestsEnabled`, не отключая стенды и локальное накопление результатов. Legacy `MirraSDKLeaderboardProvider` подчиняется тому же флагу; добавлены отсутствовавшие RU/EN ключи колонок.
- Unity compile и Play Mode прошли без ошибок. Production WebGL `Builds/ObbiZooModeration_2026-08-21_v3` завершён с `Build Finished, Result: Success`, подготовлен store ZIP `ObbiZooModeration_2026-08-21_v3_release.zip`. Для загрузки `0.0.0.3` ожидается завершение серверной проверки предыдущего архива.
