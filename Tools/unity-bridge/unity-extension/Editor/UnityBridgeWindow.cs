...
{
    /// <summary>
    /// РџСЂРѕСЃС‚РѕР№ UI РґР»СЏ СѓРїСЂР°РІР»РµРЅРёСЏ Unity Bridge
    /// РќРѕРІР°СЏ С„СѓРЅРєС†РёРѕРЅР°Р»СЊРЅР°СЏ Р°СЂС…РёС‚РµРєС‚СѓСЂР° - РјРёРЅРёРјР°Р»СЊРЅС‹Р№ РёРЅС‚РµСЂС„РµР№СЃ
    /// </summary>
    public class UnityBridgeWindow : EditorWindow
    {
        ...
        {
            // РђРІС‚РѕСЃС‚Р°СЂС‚ РїСЂРё РІРєР»СЋС‡РµРЅРёРё (РµСЃР»Рё РЅСѓР¶РЅРѕ)
        ...
        {
            // РџСЂРё РѕС‚РєР»СЋС‡РµРЅРёРё РѕРєРЅР° СЃРµСЂРІРµСЂ РїСЂРѕРґРѕР»Р¶Р°РµС‚ СЂР°Р±РѕС‚Р°С‚СЊ
        }
        ...
        private void OnGUI()
        {
            if (!EnsureStyles())
            {
                EditorGUILayout.HelpBox("Unity Editor styles are not ready yet. Repainting...", MessageType.Info);
                Repaint();
                return;
            }
            ...
            // РџРђРќР•Р›Р¬ РЈРџР РђР’Р›Р•РќРРЇ РЎР•Р Р’Р•Р РћРњ
            ...
            // РЎС‚Р°С‚СѓСЃРЅС‹Рµ СЃРѕРѕР±С‰РµРЅРёСЏ
            EditorGUILayout.Space(8);
            EditorGUILayout.BeginVertical("box");
            ...
            if (recentLogs.Length > 0)
            {
                ...
                {
                    EditorGUILayout.LabelField("вЂў " + log, EditorStyles.wordWrappedLabel);
                }
            }
            EditorGUILayout.EndScrollView();
            EditorGUILayout.EndVertical();
            
            // РўР•РЎРўР« MCP РРќРЎРўР РЈРњР•РќРўРћР’
            EditorGUILayout.Space(8);
        ...
        {
            if (stylesReady && titleStyle != null && badgeStyle != null && codeStyle != null)
                return true;
            ...
            if (bold == null || textArea == null)
                return false;

            titleStyle = new GUIStyle(bold)
            {
                fontSize = 18
            };
            badgeStyle = new GUIStyle(bold)
            {
                normal = { textColor = Color.white },
                alignment = TextAnchor.MiddleCenter
            };
            codeStyle = new GUIStyle(textArea)
            {
                fontSize = 12,
                wordWrap = false
            };
            ...
            try
            {
                var fontCandidates = Application.platform == RuntimePlatform.WindowsEditor
                    ? new[] { "Consolas", "Courier New", "Lucida Console" }
                    : new[] { "Menlo", "Monaco", "Courier New" };

                var monoFont = Font.CreateDynamicFontFromOSFont(fontCandidates, 12);
                if (monoFont != null)
                    codeStyle.font = monoFont;
            }
            catch
            {
                codeStyle.font = textArea.font;
            }
        ...
        {
            // РџСЂРёРЅСѓРґРёС‚РµР»СЊРЅРѕ РѕР±РЅРѕРІР»СЏРµРј РѕРєРЅРѕ
            Repaint();
        }
        ...
        private void TestScreenshot()
        {
            try
            {
                // РџСЂРѕСЃС‚РѕР№ С‚РµСЃС‚ - СЃРѕР·РґР°РµРј Р·Р°РїСЂРѕСЃ Рё РїСЂРѕРІРµСЂСЏРµРј РѕС‚РІРµС‚
        ...
        private void TestExecute()
        {
            try
            {
                ...
                if (result.Success)
                {
                    lastTestResult = $"Execute: SUCCESS вЂ” Result: {result.Data}";
                }
                else lastTestResult = $"Execute: FAILED вЂ” {result.Error}";
            }
            catch (System.Exception ex)
            {
                lastTestResult = $"Execute: ERROR вЂ” {ex.Message}";
            }
            UpdateStatus();
        }
